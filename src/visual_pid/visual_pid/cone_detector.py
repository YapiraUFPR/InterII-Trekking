...
def main():
    ...
    node_name = config["cone_detector"]["node"]
    topic = config["cone_detector"]["topic"]
    show_topic = config["cone_detector"]["show_topic"]
    sample_rate = config["cone_detector"]["sample_rate"]
    image_topic = config["sensors"]["camera"]["topic"]
    lower_color = config["cone_detector"]["lower_color"]
    upper_color = config["cone_detector"]["upper_color"]
    area_threshold = config["cone_detector"]["area_threshold"]
    kp = config["cone_detector"]["pid"]["kp"]
    kd = config["cone_detector"]["pid"]["kd"]
    ki = config["cone_detector"]["pid"]["ki"]
    ...
    while True:
        ...
        if len(contours) > 0:
            ...
            if largest_area > area_threshold:
                ...
                twist = Twist()
                twist.linear.x = 0.0
                twist.angular.z = correction / width
