...
class _Servo:
    ...
    def __getitem__(self, servo_channel: int) -> Servo:
        from .adafruit_motor import servo as adafruit_servo  # pylint: disable=import-outside-toplevel
        ...
        if servo is None:
            servo = adafruit_servo.Servo(self.kit._pca.channels[servo_channel])
            self.kit._items[servo_channel] = servo
            return servo
        if isinstance(self.kit._items[servo_channel], adafruit_servo.Servo):
            return servo
...
class _ContinuousServo:
    ...
    def __getitem__(self, servo_channel: int) -> ContinuousServo:
        ...
        servo = self.kit._items[servo_channel]
        if servo is None:
            servo = adafruit_servo.ContinuousServo(
                self.kit._pca.channels[servo_channel]
            )
            self.kit._items[servo_channel] = servo
            return servo
        if isinstance(
            self.kit._items[servo_channel], adafruit_servo.ContinuousServo
        ):
            return servo
        raise ValueError("Channel {} is already in use.".format(servo_channel))
